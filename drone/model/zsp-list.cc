...
ZspListPriv::GetTypeId ()
{
  static TypeId tid = TypeId ("ns3::ZspListPriv")
    .SetParent<Object> ()
    .SetGroupName("Zsps")
    .AddAttribute ("ZspList", "The list of all ZSPs created during the simulation.",
                   ObjectVectorValue (),
...
ZspListPriv::GetZsp (uint32_t n)
{
  ...
  NS_ASSERT_MSG (n < m_zsps.size (), "zsp index " << n <<
                 " is out of range (only have " << m_zsps.size () << " drones).");
