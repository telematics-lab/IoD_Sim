...
// static void
// EchoTxRx (std::string context, const Ptr< const Packet > packet, const TcpHeader &header, const
// Ptr< const TcpSocketBase > socket)
//{
//  std::cout << Simulator::Now () << ":" << context << ":" << packet->GetUid() << ":" <<
//  socket->GetNode () << ":" << header.GetSequenceNumber () << std::endl;
//}
...
main(int argc, char* argv[])
{
    ...
    std::cerr << "REMOTE=" << users.Get(1)->GetId() << ",addr=" << Ipv4Address::ConvertFrom(remote)
              << std::endl;
    ...
    std::cout << users.Get(0)->GetId() << ":" << users.Get(1)->GetId() << ": "
