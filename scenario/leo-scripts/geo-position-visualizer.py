...
def select_results_folder():
    """
    Interactive menu to select a folder from ../results containing leo-sat-trace.csv
    Returns the path to the selected leo-sat-trace.csv file
    """
    ...
    # Find folders containing leo-sat-trace.csv
    valid_folders = []
    for item in os.listdir(results_path):
        folder_path = os.path.join(results_path, item)
        if os.path.isdir(folder_path):
            csv_path = os.path.join(folder_path, "leo-sat-trace.csv")
            if os.path.exists(csv_path):
                valid_folders.append((item, csv_path))

    if not valid_folders:
        print(f"No folders containing 'leo-sat-trace.csv' found in '{results_path}'")
        return None
    ...
    while True:
        try:
            ...
            if 0 <= choice_idx < len(valid_folders):
                selected_folder, selected_path = valid_folders[choice_idx]
...
# Select the CSV file
csv_file_path = select_results_folder()
...
try:
    ...
    print(
        "Make sure the file is in the same directory as the script or specify the full path."
    )
    exit()
...
for i, nodo in enumerate(nodi_unici):
    ...
    fig.add_trace(
        go.Scatter3d(
            ...
            mode="markers",
            marker=dict(
                color=colore, size=3, opacity=1, line=dict(width=0.3, color=colore)
            ),
