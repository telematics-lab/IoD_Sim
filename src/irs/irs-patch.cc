  ...
  static TypeId tid =
      TypeId ("ns3::IrsPatch")
          ...
          .AddAttribute ("Size",
                         ...
                         IntVectorValue (), MakeIntVectorAccessor (&IrsPatch::SetSizeSerialized),
                         MakeIntVectorChecker ())
          .AddAttribute ("PhaseX",
                         "Phase angle along the X axis in case the IRS is not serving any specific "
                         "communication.",
                         DoubleValue (), MakeDoubleAccessor (&IrsPatch::m_phaseX),
                         MakeDoubleChecker<double> ())
          .AddAttribute ("PhaseY",
                         "Phase angle along the Y axis in case the IRS is not serving any specific "
                         "communication.",
                         DoubleValue (), MakeDoubleAccessor (&IrsPatch::m_phaseY),
                         MakeDoubleChecker<double> ())
          .AddAttribute ("ServingNodes", "ns-3 XPath of serving nodes.", StrVecValue (),
                         MakeStrVecAccessor (&IrsPatch::SetServingNodesSerialized),
                         MakeStrVecChecker ())
          .AddAttribute ("LifeTime", "Life time of the patch in seconds.",
                         DoubleValue (0.0), // Use 0.0 to indicate that the patch will not expire
                         MakeDoubleAccessor (&IrsPatch::SetLifeTime), MakeDoubleChecker<double> ());
  return tid;
}
