...
enum class RotoAxisType { X_AXIS = 0, Y_AXIS, Z_AXIS };
...
private:
  static const bool IsOverlapped (const uint32_t aStart, const uint32_t aEnd, const uint32_t bStart, const uint32_t bEnd);
  ...
  std::vector<RotoAxisType>
      m_rotoAxis; //Well-ordered list of axis used to set IRS rotation (or to BackRotate other points)
  std::vector<double>
      m_rotoAngles; //List of angles (in radians) used to rotate the IRS, the n-th angles refers to n-th axis defined in m_rotoAxis (or to BackRotate other points)
