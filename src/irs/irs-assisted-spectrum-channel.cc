...
IrsAssistedSpectrumChannel::GetTypeId (void)
{
  static TypeId tid =
      TypeId ("ns3::IrsAssistedSpectrumChannel")
          ...
          .AddAttribute ("OutageProbability", "Outage probability threshold", DoubleValue (1e-2),
                         MakeDoubleAccessor (&IrsAssistedSpectrumChannel::SetEps),
                         MakeDoubleChecker<double> ())
          .AddAttribute ("KMin", "Minimum k-factor for the environment, in dB", DoubleValue (6.),
                         MakeDoubleAccessor (&IrsAssistedSpectrumChannel::m_kmin),
                         MakeDoubleChecker<double> ())
          .AddAttribute ("KMax", "Maximum k-factor for the environment, in dB", DoubleValue (10.),
                         MakeDoubleAccessor (&IrsAssistedSpectrumChannel::m_kmax),
                         MakeDoubleChecker<double> ())
          .AddAttribute ("KNlos", "k-factor used in case of NLoS condition, in dB", DoubleValue (0.),
                         MakeDoubleAccessor (&IrsAssistedSpectrumChannel::m_knlos),
                         MakeDoubleChecker<double> ())
          .AddAttribute ("AlphaLoss", "Environment attenuation factor", DoubleValue (2.),
                         MakeDoubleAccessor (&IrsAssistedSpectrumChannel::m_alpha),
                         MakeDoubleChecker<double> ())
          .AddAttribute ("NoDirectLink", "Suppress the direct link with the communicating nodes",
                         BooleanValue (false),
                         MakeBooleanAccessor (&IrsAssistedSpectrumChannel::m_noDirectLink),
                         MakeBooleanChecker ())
          .AddAttribute (
              "NoIrsLink", "Suppress the IRS reflected link with the communicating nodes",
              BooleanValue (false), MakeBooleanAccessor (&IrsAssistedSpectrumChannel::m_noIrsLink),
              MakeBooleanChecker ())
          .AddAttribute ("MultipathInterference",
                         ...
                         MakeEnumChecker (MultipathInterferenceType::DESTRUCTIVE, "DESTRUCTIVE",
                                          MultipathInterferenceType::SIMULATED, "SIMULATED",
                                          MultipathInterferenceType::CONSTRUCTIVE, "CONSTRUCTIVE"));
  return tid;
}
...
IrsAssistedSpectrumChannel::StartTx (Ptr<SpectrumSignalParameters> txParams)
{
  ...
  NS_LOG_LOGIC (
      "converter map size: " << txInfoIt->second.m_spectrumConverterMap.size ());
  NS_LOG_LOGIC ("converter map first element: "
                << txInfoIt->second.m_spectrumConverterMap.begin ()->first);
  ...
  for (auto &rxInfo : GetRxSpectrumModelInfoMap ())
    {
      ...
      if (txSpectrumModelUid == rxSpectrumModelUid)
      ...
      else
        {
          ...
          auto rxConverterIt =
              txInfoIt->second.m_spectrumConverterMap.find (rxSpectrumModelUid); // TODO: const ref?
      ...
      for (auto &rxPhy : rxInfo.second.m_rxPhys)
        {
          ...
          for (uint32_t j = 0; j < n_irs; ++j)
            {
              F_BRG = 1;
              IrsPosition = IrsList::Get(j)->GetDrone()->GetObject<MobilityModel>()->GetPosition();
              ...
              if (!m_noIrsLink)
                {
                  etav_tmp.push_back (std::sqrt (beta_BRG[j] * std::pow (d_BR[j] * d_RG[i][j], -2.) * F_BRG));
                }
              else
                {
      ...
      const auto gain = GetGain (f_c, n_users, n_irs, d_BG, K_BR, K_RG, etav, lambdav, d_BR, d_RG,
                                 a_BR, a_RG, K_BG_nu, K_BG_sigma);
              ...
              if (rxNetDevice)
                {
                  ...
                  Simulator::ScheduleWithContext (
                      dstNode, delay, &IrsAssistedSpectrumChannel::StartRx, this, rxParams, rxPhy);
                }
              else
                {
                  // the receiver is not attached to a NetDevice, so we cannot assume that it is attached to a node
                  Simulator::Schedule (delay, &IrsAssistedSpectrumChannel::StartRx, this, rxParams,
                                       rxPhy);
                }
            }
          ++u;
...
std::vector<double>
IrsAssistedSpectrumChannel::GetGain (
    const double f_c, const int n_users, const int n_irs, const std::vector<double> &d_BG,
    const std::vector<double> &K_BR, const std::vector<std::vector<double>> &K_RG,
    const std::vector<std::vector<double>> &etav, const std::vector<double> &lambdav,
    const std::vector<double> &d_BR, const std::vector<std::vector<double>> &d_RG,
    const std::vector<Angles> &a_BR, const std::vector<std::vector<Angles>> &a_RG,
    const std::vector<double> &K_BG_nu, const std::vector<double> &K_BG_sigma)
{
          ...
          for (std::size_t p = 0; p < P; ++p)
            {
                      ...
                      if (!std::isnan (phi_o))
                        {
                          ...
                          theta_r = std::asin ((phaseY - std::sin (theta_o) * std::sin (phi_o)) / std::sin (phi_r));
                        }
                      else
                        {
                  ...
                  if (!std::isnan (theta_r))
                    {
                      ...
                      distance = std::sqrt (std::pow (z, 2.) + std::pow (z * std::tan (theta_r), 2.)) + d_BR[d];
                    }
                  else
                    {
              ...
              if (modf (phi_x * M_PI * f_c / SPEED_OF_LIGHT, &pigr) ==
                ...
                modules_tmp[p] =
                    modules_tmp[p] * patches[p]->GetSize ().GetColSize (); //Paper: Formula 19 (Chi)
              else
                modules_tmp[p] =
                    modules_tmp[p] *
              ...
              phases_tmp.push_back (-2. * M_PI * f_c / SPEED_OF_LIGHT *
          ...
          for (std::size_t p = 0; p < P; ++p)
            {
              ...
              sig_BRG += std::pow (etav[u][d], 2.) * std::pow (K_BRG_sigma, 2.) *
                         patches[p]->GetSize ().GetRowSize () *
                         patches[p]->GetSize ().GetColSize ();
            }
        }
      ...
      else
        {
          ...
          if (std::sqrt (2. * K) > 2.5848)
            gain.push_back (std::pow (std::sqrt (2. * K) +
                                     0.5 / m_invqfunc *
                                         std::log (std::sqrt (2. * K) / (std::sqrt (2. * K) - m_invqfunc)) -
                                     m_invqfunc,
                                 2.) /
                            (2. * (K + 1.) / sigma));
          else
...
void
IrsAssistedSpectrumChannel::NodeToIrssAngles (Ptr<MobilityModel> node,
                                              std::vector<Angles> &a_Vector)
{
...
void
IrsAssistedSpectrumChannel::GetServedParam (Ptr<Irs> irs, Ptr<IrsPatch> patch, double &phaseY,
                                            double &phaseX, double &distance)
{
