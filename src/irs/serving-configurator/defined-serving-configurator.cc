...
{
  static TypeId tid = TypeId ("ns3::DefinedServingConfigurator")
      ...
      if (periodend <= periodstart + nextupdate)
        {
          ...
          //Simulator::Schedule (Seconds (periodstart + nextupdate), &DefinedServingConfigurator::DoDispose, this);
          break; //we have reached the end of the period, no need to continue
        }
      if (nextupdate > 0)
        {
          Simulator::Schedule (Seconds (nextupdate),
                               &DefinedServingConfigurator::UpdateServingNodes, this,
                               m_servingpairs.at (j));
        }
      else
        {
...
void
DefinedServingConfigurator::SetPeriods (const DoubleVector &a)
{
