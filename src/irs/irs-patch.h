...
public:
  ...
  public:
    Size () :
      ...
      endRowIdx   {0}
    {}
    Size (uint32_t startColIdx, uint32_t endColIdx,
    ...
    const uint32_t GetRowSize () const { return endRowIdx - startRowIdx + 1; }
    const uint32_t GetColSize () const { return endColIdx - startColIdx + 1; }
...
private:
  ...
  double m_phaseX;      /// Phase shift on the X-axis used if the patch is not serving any PhyLayer (or Node)
  double m_phaseY;      /// Phase shift on the Y-axis used if the patch is not serving any PhyLayer (or Node)
