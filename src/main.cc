...
Scenario::operator() ()
{
  ...
  if (CONFIGURATOR->RadioMap () > 0)
    {
      NS_ASSERT_MSG (anyLte, "Environment Radio Map can be generated only if an LTE network is "
...
Scenario::GenerateRadioMap ()
{
  ...
  m_remHelper->SetAttribute ("OutputFile", StringValue (CONFIGURATOR->GetResultsPath () +
                                                        CONFIGURATOR->GetName () + "-2D-REM.txt"));
        ...
        else if (entityNetDev->GetType () == "lte")
          {
            ...
            switch (role)
              ...
              case eNB:
                ...
                break;
              case UE:
                ConfigureLteUe (entityNode, entityLteDevConf->GetBearers (),
                                netId, antennaModel, phyConf);
                break;
              default:
  ...
  for (auto perConf : conf->GetPeripherals ())
    {
      ...
      for (auto aggIt = perConf.AggregatesBegin (); aggIt != perConf.AggregatesEnd (); aggIt++)
        {
          NS_LOG_INFO ("Aggregating " << aggIt->GetName () << " to " << peripheral->GetTypeId ().GetName ());
