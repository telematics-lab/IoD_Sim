...
namespace ns3
{
    ...
    TypeId DroneClientApplication::GetTypeId()
    {
        static TypeId tid =
    ...
    void DroneClientApplication::DoDispose()
    {
    ...
    void DroneClientApplication::StartApplication()
    {
        NS_LOG_FUNCTION(this);
    ...
    void DroneClientApplication::SendPacket(const Intent i,
    ...
    void DroneClientApplication::ReceivePacket(const Ptr<Socket> socket)
    {
        ...
        while ((packet = socket->RecvFrom(senderAddr)))
        {
            if (InetSocketAddress::IsMatchingType(senderAddr))
            {
                ...
                NS_LOG_INFO("[Node " << GetNode()->GetId() << "] client received "
                                     << packet->GetSize() << " bytes from " << senderIpv4);
    ...
    void DroneClientApplication::CourseChange(const std::string context,
