...
DroneServerApplication::ReceivePacket(const Ptr<Socket> socket) const
{
    ...
    while ((packet = socket->RecvFrom(senderAddr)))
    {
        if (InetSocketAddress::IsMatchingType(senderAddr))
        {
            ...
            std::cout << "Server: " << plainpayload << std::endl;
                ...
                m_storage)
            {
                ...
                if (storage->Alloc(strlen((char*)plainpayload) * sizeof(char),
                                   StoragePeripheral::byte))
                    NS_LOG_INFO("[Node " << GetNode()->GetId() << "] Stored "
                                         << strlen((char*)plainpayload) * sizeof(char)
                                         << " bytes ");
            }
            ...
            else
            {
                ...
                case PacketType::HELLO: {
                    ...
                    myX509 =
                        x509Obj.load_x509_certificate("./src/iodsim/certificates/client_x509.pem");
                    myPkey =
                        x509Obj.load_key("./src/iodsim/certificates/client_public_key.pem", false);
                    int rc = X509_verify(myX509, myPkey);
                    if (rc != 1)
                    {
                        std::cout << "Server: X509_verify()  Error" << std::endl;
                    }
                    else
                    {
                        m_sendEvent = Simulator::ScheduleNow(&DroneServerApplication::SendHelloAck,
                                                             ...
                                                             senderIpv4,
                                                             senderPort);
                        std::cout << "Server: X509_verify()  Success" << std::endl;
                    }
                }
                break;
                                     ...
                                     const uint32_t senderPort) const
{
    ...
    const auto packet =
        Create<Packet>((const uint8_t*)cipherjson, strlen((const char*)cipherjson) * sizeof(char));
