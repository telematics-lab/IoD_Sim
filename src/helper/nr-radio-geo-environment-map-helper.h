...
#ifndef NR_RADIO_GEO_ENVIRONMENT_MAP_HELPER_H
#define NR_RADIO_GEO_ENVIRONMENT_MAP_HELPER_H
 ...
 * and IPSD.
 *
 * The NrRadioGeoEnvironmentMapHelper gives the possibility to generate maps either
...
class NrRadioGeoEnvironmentMapHelper : public Object
{
  public:
    enum RemMode
    {
    ...
    /**
     * @brief NrRadioGeoEnvironmentMapHelper constructor
     */
    NrRadioGeoEnvironmentMapHelper();

    /**
     * @brief destructor
     */
    ~NrRadioGeoEnvironmentMapHelper() override;
    ...
    /**
     * @brief Sets the min longitude of the map
     * @param minLon The min longitude
     */
    void SetMinLon(double minLon);

    /**
     * @brief Sets the min latitude of the map
     * @param minLat The min latitude
     */
    void SetMinLat(double minLat);

    /**
     * @brief Sets the max longitude of the map
     * @param maxLon The max longitude
     */
    void SetMaxLon(double maxLon);

    /**
     * @brief Sets the max latitude of the map
     * @param maxLat The max latitude
     */
    void SetMaxLat(double maxLat);

    /**
     * @brief Sets the resolution (number of points) of the map along the longitude
     * @param xRes The longitude resolution
     */
    void SetResX(uint16_t xRes);

    /**
     * @brief Sets the resolution (number of points) of the map along the latitude
     * @param yRes The latitude resolution
     */
    void SetResY(uint16_t yRes);

    /**
     * @brief Sets the altitude of the map
     ...
     */
    /**
     * @return Gets the value of the min longitude of the map
     */
    double GetMinLon() const;

    /**
     * @return Gets the value of the min latitude of the map
     */
    double GetMinLat() const;

    /**
     * @return Gets the value of the max longitude of the map
     */
    double GetMaxLon() const;

    /**
     * @return Gets the value of the max latitude of the map
     */
    double GetMaxLat() const;

    /**
     * @return Gets the value of the resolution (number of points)
     * of the map along the longitude
     */
    uint16_t GetResX() const;

    /**
     * @return Gets the value of the resolution (number of points)
     * of the map along the latitude
     */
    uint16_t GetResY() const;

    /**
     * @return Gets the value of the altitude of the map
     */
    ...
    /**
     * @brief Calculate the SINR for a specific UE and gNB at the current time.
     ...
     */
    double GetSinr(Ptr<NetDevice> ueDevice,
  ...
  private:
    ...
    struct RemDevice
    {
        ...
        RemDevice()
        {
            ...
            mob = node->GetObject<GeocentricMobilityModel>();
        }
    };
     ...
     * the min/max coordinates and the resolution defined by the user
     */
    ...
    /**
     * @brief this method prints the REM points in Geocentric coordinates (X, Y, Z)
     * along with the SINR values.
     */
    void PrintGeocentricRemToFile();
    ...
    double m_minLon{0};  ///< The `MinLon` attribute.
    double m_maxLon{0};  ///< The `MaxLon` attribute.
    uint16_t m_xRes{0};  ///< The `XRes` attribute. (Longitude resolution)
    double m_lonStep{0}; ///< Distance along Longitude axis between adjacent listening points.

    double m_minLat{0};   ///< The `MinLat` attribute.
    double m_maxLat{0};   ///< The `MaxLat` attribute.
    uint16_t m_yRes{0};   ///< The `YRes` attribute. (Latitude resolution)
    double m_latStep{0};  ///< Distance along Latitude axis between adjacent listening points.
    double m_altitude{0}; ///< The `Altitude` attribute.
...
public:
     ...
     */
    void SetLogGeocentricRem(bool logGeocentricRem);

    /**
     * @brief Gets the attribute LogGeocentricRem
...
}; // end of `class NrRadioGeoEnvironmentMapHelper`

} // namespace ns3

#endif // NR_RADIO_GEO_ENVIRONMENT_MAP_HELPER_H
