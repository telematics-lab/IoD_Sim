...
#include "three-dimensional-rem-helper.h"
...
NS_LOG_COMPONENT_DEFINE("ThreeDimensionalRemHelper");
...
TypeId
ThreeDimensionalRemHelper::GetTypeId()
{
    NS_LOG_FUNCTION("ThreeDimensionalRemHelper::GetTypeId");
    static TypeId tid =
        TypeId("ns3::ThreeDimensionalRemHelper")
            .SetParent<Object>()
            .SetGroupName("Lte")
            .AddConstructor<ThreeDimensionalRemHelper>()
            .AddAttribute(
                "Channel",
                ...
                MakePointerAccessor(&ThreeDimensionalRemHelper::m_channel),
            ...
            .AddAttribute(
                ...
                MakeStringAccessor(&ThreeDimensionalRemHelper::m_channelPath),
                MakeStringChecker())
            .AddAttribute("OutputFile",
                          "the filename to which the Radio Environment Map is saved",
                          StringValue("rem.out"),
                          MakeStringAccessor(&ThreeDimensionalRemHelper::m_outputFile),
                          MakeStringChecker())
            .AddAttribute("XMin",
                          "The min x coordinate of the map.",
                          DoubleValue(0.0),
                          MakeDoubleAccessor(&ThreeDimensionalRemHelper::m_xMin),
                          MakeDoubleChecker<double>())
            .AddAttribute("YMin",
                          "The min y coordinate of the map.",
                          DoubleValue(0.0),
                          MakeDoubleAccessor(&ThreeDimensionalRemHelper::m_yMin),
                          MakeDoubleChecker<double>())
            .AddAttribute("ZMin",
                          "The min z coordinate of the map.",
                          DoubleValue(0.0),
                          MakeDoubleAccessor(&ThreeDimensionalRemHelper::m_zMin),
                          MakeDoubleChecker<double>())
            .AddAttribute("XMax",
                          "The max x coordinate of the map.",
                          DoubleValue(1.0),
                          MakeDoubleAccessor(&ThreeDimensionalRemHelper::m_xMax),
                          MakeDoubleChecker<double>())
            .AddAttribute("YMax",
                          "The max y coordinate of the map.",
                          DoubleValue(1.0),
                          MakeDoubleAccessor(&ThreeDimensionalRemHelper::m_yMax),
                          MakeDoubleChecker<double>())
            .AddAttribute("ZMax",
                          "The max z coordinate of the map.",
                          DoubleValue(1.0),
                          MakeDoubleAccessor(&ThreeDimensionalRemHelper::m_zMax),
                          MakeDoubleChecker<double>())
            .AddAttribute("XRes",
                          "The resolution (number of points) of the map along the x axis.",
                          UintegerValue(100),
                          MakeUintegerAccessor(&ThreeDimensionalRemHelper::m_xRes),
            ...
            .AddAttribute("YRes",
                          ...
                          UintegerValue(100),
                          MakeUintegerAccessor(&ThreeDimensionalRemHelper::m_yRes),
                          MakeUintegerChecker<uint16_t>(2, std::numeric_limits<uint16_t>::max()))
            .AddAttribute("ZRes",
                          "The resolution (number of points) of the map along the z axis.",
                          UintegerValue(100),
            ...
            .AddAttribute(
                ...
                BooleanValue(true),
                MakeBooleanAccessor(&ThreeDimensionalRemHelper::m_stopWhenDone),
                MakeBooleanChecker())
            .AddAttribute("NoisePower",
                          "the power of the measuring instrument noise, in Watts. Default to a kT "
                          "of -174 dBm "
                          "with a noise figure of 9 dB and a bandwidth of 25 LTE Resource Blocks",
                          DoubleValue(1.4230e-13),
                          MakeDoubleAccessor(&ThreeDimensionalRemHelper::m_noisePower),
                          MakeDoubleChecker<double>())
            .AddAttribute("PointsPerIteration",
                          "Number of REM points to be calculated per iteration. Every point "
                          "consumes approximately 5KB of memory.",
                          UintegerValue(20000),
                          MakeUintegerAccessor(&ThreeDimensionalRemHelper::m_pointsPerIteration),
                          MakeUintegerChecker<uint32_t>(1, std::numeric_limits<uint32_t>::max()))
            .AddAttribute("Earfcn",
                          "E-UTRA Absolute Radio Frequency Channel Number (EARFCN) "
                          "as per 3GPP 36.101 Section 5.7.3. ",
                          UintegerValue(100),
                          MakeUintegerAccessor(&ThreeDimensionalRemHelper::m_earfcn),
                          MakeUintegerChecker<uint16_t>())
            .AddAttribute("Bandwidth",
                          "Transmission Bandwidth Configuration (in number of RBs) over which the "
                          "SINR will be calculated",
                          UintegerValue(25),
                          MakeUintegerAccessor(&ThreeDimensionalRemHelper::SetBandwidth,
                                               &ThreeDimensionalRemHelper::GetBandwidth),
                          MakeUintegerChecker<uint16_t>())
            .AddAttribute("UseDataChannel",
                          "If true, REM will be generated for PDSCH and for PDCCH otherwise ",
                          BooleanValue(false),
                          MakeBooleanAccessor(&ThreeDimensionalRemHelper::m_useDataChannel),
                          MakeBooleanChecker())
            .AddAttribute("RbId",
...
uint16_t
ThreeDimensionalRemHelper::GetBandwidth() const
{
    return m_bandwidth;
}

void
ThreeDimensionalRemHelper::SetBandwidth(uint16_t bw)
{
    switch (bw)
    {
    case 6:
...
void
ThreeDimensionalRemHelper::Install()
{
    NS_LOG_FUNCTION(this);
    if (!m_rem.empty())
    {
    ...
    Simulator::Schedule(Seconds(startDelay), &ThreeDimensionalRemHelper::DelayedInstall, this);
}

void
ThreeDimensionalRemHelper::DelayedInstall()
{
    NS_LOG_FUNCTION(this);
    m_xStep = (m_xMax - m_xMin) / (m_xRes - 1);
    m_yStep = (m_yMax - m_yMin) / (m_yRes - 1);
    m_zStep = (m_zMax - m_zMin) / (m_zRes - 1);

    if (((double)m_xRes * (double)m_yRes < (double)m_pointsPerIteration) ||
        !((m_xRes * m_yRes) % m_pointsPerIteration == 0))
    {
        std::cout << "PointsPerIteration value not valid, setting it to " << (m_xRes * m_yRes)
                  << std::endl;
        m_pointsPerIteration = m_xRes * m_yRes;
    }

    for (uint32_t i = 0; i < m_pointsPerIteration; ++i)
    {
        RemPoint p;
    ...
    for (double z = m_zMin; z < m_zMax + 0.5 * m_zStep; z += m_zStep)
    {
        for (double x = m_xMin; x < m_xMax + 0.5 * m_xStep; x += m_xStep)
        {
            for (double y = m_yMin; y < m_yMax + 0.5 * m_yStep; y += m_yStep)
            {
                ...
                if ((numPointsCurrentIteration == m_pointsPerIteration) ||
                    ((x > m_xMax - 0.5 * m_xStep) && (y > m_yMax - 0.5 * m_yStep)))
                {
                    Simulator::Schedule(Seconds(remIterationStartTime),
                                        &ThreeDimensionalRemHelper::RunOneIteration,
                                        this,
                                        xMinNext,
                                        x,
                                        yMinNext,
    ...
    Simulator::Schedule(Seconds(remIterationStartTime), &ThreeDimensionalRemHelper::Finalize, this);
}

void
ThreeDimensionalRemHelper::RunOneIteration(double xMin,
                                           double xMax,
                                           double yMin,
                                           double yMax,
                                           double z)
{
    ...
    for (x = xMin; x < xMax + 0.5 * m_xStep; x += m_xStep)
    {
             ...
             y += m_yStep)
        {
            ...
            remIt->bmm->SetPosition(Vector(x, y, z));
    ...
    if (remIt != m_rem.end())
    {
        ...
        NS_FATAL_ERROR("Some points were not used, something went wrong, check the attribute "
                       "m_pointsPerIteration");
    }

    Simulator::Schedule(Seconds(0.0005), &ThreeDimensionalRemHelper::PrintAndReset, this);
}

void
ThreeDimensionalRemHelper::PrintAndReset()
{
    NS_LOG_FUNCTION(this);

    for (std::list<RemPoint>::iterator it = m_rem.begin(); it != m_rem.end(); ++it)
    {
        if (std::isgreaterequal(it->phy->GetSinr(m_noisePower), m_threshold))
        {
...
void
ThreeDimensionalRemHelper::Finalize()
{
    NS_LOG_FUNCTION(this);
    m_outFile.close();
    if (m_stopWhenDone)
