...
class DebugHelper
{
  public:
    static void ProbeNodes()
    {
        ...
        std::cout << "There are " << Config::GetRootNamespaceObjectN() << " root objects available."
                  << std::endl;
    ...
    static void ObjectAttributes(Ptr<Object> o)
    {
        for (size_t n = 0; n < o->GetInstanceTypeId().GetParent().GetAttributeN(); n++)
        {
            TypeId::AttributeInformation attrInfo =
                o->GetInstanceTypeId().GetParent().GetAttribute(n);
            ...
            std::cout << attrName << " " << typeidvalue << " "
                      << attrInfo.initialValue->SerializeToString(attrInfo.checker) << std::endl;
        }
