...
#ifndef RAPIDYYJSON_PRETTYWRITER_H_
#define RAPIDYYJSON_PRETTYWRITER_H_
...
class PrettyWriter {
public:
    PrettyWriter(OutputStream& os) : m_os(os), m_doc(yyjson_mut_doc_new(nullptr)), m_current_key(nullptr) {}
    ~PrettyWriter() { if (m_doc) yyjson_mut_doc_free(m_doc); }
...
private:
   ...
   void CheckRoot() {
       if (m_stack.empty()) {
           // Root is complete, serialize
           char* json = yyjson_mut_write(m_doc, YYJSON_WRITE_PRETTY, nullptr);
           if (json) {
               m_os.Append(json);
               free(json);
           }
...
#endif // RAPIDYYJSON_PRETTYWRITER_H_
