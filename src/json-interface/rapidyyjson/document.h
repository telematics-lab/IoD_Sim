...
{
  public:
    Allocator(yyjson_mut_doc* doc = nullptr)
        : m_doc(doc)
    {
    }
    ...
    void SetDoc(yyjson_mut_doc* doc)
    {
        m_doc = doc;
    }
...
class GenericArray
{
  public:
    ...
    GenericArray(const ValueT& value)
        : m_value(const_cast<ValueT&>(value))
    {
    }

    template <bool SourceConst>
    GenericArray(const GenericArray<SourceConst, ValueT>& source)
        : m_value((ValueT&)source)
    {
    }
...
class GenericObject
{
  public:
    ...
    GenericObject(const ValueT& value)
        : m_value(const_cast<ValueT&>(value))
    {
    }

    template <bool SourceConst>
    GenericObject(const GenericObject<SourceConst, ValueT>& source)
        : m_value((ValueT&)source)
    {
    }
...
class Value
{
  public:
    ...
    void Attach(yyjson_mut_val* val)
    {
        m_val = val;
        UpdateTypeFromVal();
    }
    ...
    bool IsNull() const
    {
        return m_type == kNullType;
    }

    bool IsFalse() const
    {
        return m_type == kFalseType;
    }

    bool IsTrue() const
    {
        return m_type == kTrueType;
    }

    bool IsBool() const
    {
        return m_type == kFalseType || m_type == kTrueType;
    }

    bool IsObject() const
    {
        return m_type == kObjectType;
    }

    bool IsArray() const
    {
        return m_type == kArrayType;
    }

    bool IsNumber() const
    {
        return m_type == kNumberType;
    }

    bool IsString() const
    {
        return m_type == kStringType;
    }
    ...
    bool IsUint() const
    {
        ...
        if (m_val)
        {
            ...
            {
                double d = yyjson_mut_get_real(m_val);
                return d >= 0 && d <= static_cast<double>(std::numeric_limits<uint32_t>::max()) &&
    ...
    bool IsUint64() const
    {
        ...
        if (m_val)
        {
            ...
            {
                double d = yyjson_mut_get_real(m_val);
                return d >= 0 && d <= static_cast<double>(std::numeric_limits<uint64_t>::max()) &&
    ...
    bool GetBool() const
    {
        ...
        if (std::holds_alternative<bool>(m_detached))
            return std::get<bool>(m_detached);
        return false;
    }
    ...
    int GetInt() const
    {
        ...
        {
            if (yyjson_mut_is_int(m_val))
                return (int)yyjson_mut_get_sint(m_val);
            if (yyjson_mut_is_uint(m_val))
                return (int)yyjson_mut_get_uint(m_val);
            if (yyjson_mut_is_real(m_val))
                return (int)yyjson_mut_get_real(m_val);
            return 0;
        }
        if (std::holds_alternative<int64_t>(m_detached))
            return (int)std::get<int64_t>(m_detached);
        if (std::holds_alternative<uint64_t>(m_detached))
            return (int)std::get<uint64_t>(m_detached);
        if (std::holds_alternative<double>(m_detached))
            return (int)std::get<double>(m_detached);
        return 0;
    }
    ...
    unsigned GetUint() const
    {
        ...
        {
            if (yyjson_mut_is_uint(m_val))
                return (unsigned)yyjson_mut_get_uint(m_val);
            if (yyjson_mut_is_int(m_val))
                return (unsigned)yyjson_mut_get_sint(m_val);
            if (yyjson_mut_is_real(m_val))
                return (unsigned)yyjson_mut_get_real(m_val);
            return 0;
        }
        if (std::holds_alternative<int64_t>(m_detached))
            return (unsigned)std::get<int64_t>(m_detached);
        if (std::holds_alternative<uint64_t>(m_detached))
            return (unsigned)std::get<uint64_t>(m_detached);
        if (std::holds_alternative<double>(m_detached))
            return (unsigned)std::get<double>(m_detached);
        return 0;
    }
    ...
    int64_t GetInt64() const
    {
        ...
        {
            if (yyjson_mut_is_int(m_val))
                return yyjson_mut_get_sint(m_val);
            if (yyjson_mut_is_uint(m_val))
                return (int64_t)yyjson_mut_get_uint(m_val);
            if (yyjson_mut_is_real(m_val))
                return (int64_t)yyjson_mut_get_real(m_val);
            return 0;
        }
        if (std::holds_alternative<int64_t>(m_detached))
            return std::get<int64_t>(m_detached);
        if (std::holds_alternative<uint64_t>(m_detached))
            return (int64_t)std::get<uint64_t>(m_detached);
        if (std::holds_alternative<double>(m_detached))
            return (int64_t)std::get<double>(m_detached);
        return 0;
    }
    ...
    uint64_t GetUint64() const
    {
        ...
        {
            if (yyjson_mut_is_uint(m_val))
                return yyjson_mut_get_uint(m_val);
            if (yyjson_mut_is_int(m_val))
                return (uint64_t)yyjson_mut_get_sint(m_val);
            if (yyjson_mut_is_real(m_val))
                return (uint64_t)yyjson_mut_get_real(m_val);
            return 0;
        }
        if (std::holds_alternative<int64_t>(m_detached))
            return (uint64_t)std::get<int64_t>(m_detached);
        if (std::holds_alternative<uint64_t>(m_detached))
            return std::get<uint64_t>(m_detached);
        if (std::holds_alternative<double>(m_detached))
            return (uint64_t)std::get<double>(m_detached);
        return 0;
    }
    ...
    double GetDouble() const
    {
        ...
        {
            if (yyjson_mut_is_real(m_val))
                return yyjson_mut_get_real(m_val);
            if (yyjson_mut_is_int(m_val))
                return (double)yyjson_mut_get_sint(m_val);
            if (yyjson_mut_is_uint(m_val))
                return (double)yyjson_mut_get_uint(m_val);
            return 0.0;
        }
        if (std::holds_alternative<double>(m_detached))
            return std::get<double>(m_detached);
        if (std::holds_alternative<int64_t>(m_detached))
            return (double)std::get<int64_t>(m_detached);
        if (std::holds_alternative<uint64_t>(m_detached))
            return (double)std::get<uint64_t>(m_detached);
        return 0.0;
    }
    ...
    const char* GetString() const
    {
        ...
        if (std::holds_alternative<std::string>(m_detached))
            return std::get<std::string>(m_detached).c_str();
        return "";
    }
    ...
    std::size_t GetStringLength() const
    {
        ...
        if (std::holds_alternative<std::string>(m_detached))
            return std::get<std::string>(m_detached).size();
        return 0;
    }
    ...
    void SetString(const char* s, SizeType len, Allocator& allocator)
    {
        m_type = kStringType;
        if (m_val)
            yyjson_mut_set_strn(m_val, s, len);
        else
            m_detached = std::string(s, len);
    }
    ...
    GenericArray<false, Value> GetArray()
    {
        return GenericArray<false, Value>(*this);
    }
    ...
    GenericObject<false, Value> GetObject()
    {
        return GenericObject<false, Value>(*this);
    }
    ...
    using DetachedData = std::variant<std::monostate,
                                      bool,
                                      int64_t,
                                      uint64_t,
                                      double,
...
class GenericMemberIterator
{
  ...
  public:
    GenericMemberIterator(yyjson_mut_val* obj)
        : m_detachedObj(nullptr),
          m_detachedIdx(0)
    {
    ...
    GenericMemberIterator(Value::ObjectData* obj)
        : m_detachedObj(obj),
          m_detachedIdx(0)
    {
        if (obj)
    ...
    bool operator!=(const GenericMemberIterator& rhs) const
    {
        return m_has_next != rhs.m_has_next;
    }
    ...
    GenericMemberIterator operator++(int)
    {
        GenericMemberIterator tmp = *this;
        Advance();
        return tmp;
    }
...
class GenericArrayIterator
{
  ...
  public:
    ...
    GenericArrayIterator(Value* parent, size_t idx = 0)
        : m_parent(parent),
          m_idx(idx)
    {
    }
    ...
    GenericArrayIterator operator+(difference_type n) const
    {
        return GenericArrayIterator(m_parent, m_idx + n);
    }
    ...
    GenericArrayIterator& operator+=(difference_type n)
    {
        m_idx += n;
        return *this;
    }
    ...
    {
        return m_parent == rhs.m_parent && m_idx == rhs.m_idx;
    }
...
inline GenericArrayIterator
operator+(std::ptrdiff_t n, const GenericArrayIterator& it)
{
    return it + n;
}
...
template <bool Const, typename ValueT>
inline typename GenericArray<Const, ValueT>::ValueIterator
GenericArray<Const, ValueT>::Begin() const
{
...
template <bool Const, typename ValueT>
inline typename GenericArray<Const, ValueT>::ValueIterator
GenericArray<Const, ValueT>::End() const
{
    return m_value.End();
}
...
template <bool Const, typename ValueT>
inline SizeType
GenericArray<Const, ValueT>::Size() const
{
    return m_value.Size();
}
...
template <bool Const, typename ValueT>
inline const ValueT&
GenericArray<Const, ValueT>::operator[](SizeType index) const
{
    return m_value[index];
}

template <bool Const, typename ValueT>
inline typename GenericObject<Const, ValueT>::MemberIterator
GenericObject<Const, ValueT>::MemberBegin() const
{
...
template <bool Const, typename ValueT>
inline typename GenericObject<Const, ValueT>::MemberIterator
GenericObject<Const, ValueT>::MemberEnd() const
{
...
template <bool Const, typename ValueT>
inline typename GenericObject<Const, ValueT>::MemberIterator
GenericObject<Const, ValueT>::begin() const
{
    return m_value.MemberBegin();
}
...
template <bool Const, typename ValueT>
inline typename GenericObject<Const, ValueT>::MemberIterator
GenericObject<Const, ValueT>::end() const
{
    return m_value.MemberEnd();
}
...
template <bool Const, typename ValueT>
inline SizeType
GenericObject<Const, ValueT>::MemberCount() const
{
...
template <bool Const, typename ValueT>
inline bool
GenericObject<Const, ValueT>::HasMember(const char* name) const
{
...
template <bool Const, typename ValueT>
inline const ValueT&
GenericObject<Const, ValueT>::operator[](const char* name) const
{
    return m_value[name];
}

inline Value::Value(const Value& rhs, Allocator& allocator)
    : m_type(rhs.m_type),
      m_val(nullptr)
{
    if (rhs.m_val)
    {
        if (rhs.IsString())
            m_detached = std::string(rhs.GetString());
        else if (rhs.IsInt())
            m_detached = (int64_t)rhs.GetInt();
        else if (rhs.IsDouble())
            m_detached = rhs.GetDouble();
        else if (rhs.IsBool())
            m_detached = rhs.GetBool();
        else if (rhs.IsArray())
        {
            m_detached = ArrayData{};
...
inline void
Value::AddMember(Value& name, Value& value, Allocator& allocator)
{
    if (!m_val)
    {
...
inline void
Value::AddMember(const char* name, Value& value, Allocator& allocator)
{
...
inline GenericMemberIterator
Value::MemberBegin()
{
    if (m_val)
        return GenericMemberIterator(m_val);
    if (std::holds_alternative<ObjectData>(m_detached))
        return GenericMemberIterator(&std::get<ObjectData>(m_detached));
...
inline GenericMemberIterator
Value::MemberBegin() const
{
    return const_cast<Value*>(this)->MemberBegin();
}

inline GenericMemberIterator
Value::MemberEnd() const
{
...
Value::Begin()
{
    return GenericArrayIterator(this, 0);
}

inline GenericArrayIterator
Value::End()
{
    return GenericArrayIterator(this, Size());
}
...
Value::End() const
{
    return const_cast<Value*>(this)->End();
}
...
inline bool
Value::HasMember(const char* name) const
{
    bool found = false;
    if (m_val)
        found = (yyjson_mut_obj_get(m_val, name) != nullptr);
...
inline void
Value::RemoveMember(const char* name)
{
    if (m_val)
        yyjson_mut_obj_remove_key(m_val, name);
    else if (std::holds_alternative<ObjectData>(m_detached))
    {
        auto& obj = std::get<ObjectData>(m_detached);
...
{
    if (!m_children)
        m_children = std::make_shared<ChildrenCache>();
    ...
    Value v;
    if (m_val)
        v = Value(yyjson_mut_obj_get(m_val, name));
    else if (std::holds_alternative<ObjectData>(m_detached))
    {
        const auto& obj = std::get<ObjectData>(m_detached);
        for (const auto& kv : obj)
...
{
    // Note: const access still needs caching if we want to return const reference to ephemeral
    // value? Actually, legacy code expects `const Value& v = arr[0]`. If we return Value by value,
    // it binds to const ref, but lifetime ends at statement. If we return const Value&, we MUST
    // cache.
    if (!m_children)
        m_children = std::make_shared<ChildrenCache>();
    ...
    Value v;
    if (m_val)
        v = Value(yyjson_mut_arr_get(m_val, index));
...
{
    if (m_val)
        return (SizeType)yyjson_mut_get_len(m_val);
    if (std::holds_alternative<ArrayData>(m_detached))
        return std::get<ArrayData>(m_detached).size();
    return 0;
}

inline SizeType
Value::MemberCount() const
{
    if (m_val && yyjson_mut_is_obj(m_val))
        return (SizeType)yyjson_mut_get_len(m_val);
    if (std::holds_alternative<ObjectData>(m_detached))
        return std::get<ObjectData>(m_detached).size();
    return 0;
}

inline void
Value::PushBack(Value& value, Allocator& allocator)
{
    if (m_val)
    {
...
inline yyjson_mut_val*
Value::ToYYVal(yyjson_mut_doc* doc)
{
    if (m_val)
        return m_val;
    if (std::holds_alternative<int64_t>(m_detached))
        return yyjson_mut_sint(doc, std::get<int64_t>(m_detached));
    if (std::holds_alternative<uint64_t>(m_detached))
        return yyjson_mut_uint(doc, std::get<uint64_t>(m_detached));
    if (std::holds_alternative<double>(m_detached))
        return yyjson_mut_real(doc, std::get<double>(m_detached));
    if (std::holds_alternative<std::string>(m_detached))
        return yyjson_mut_str(doc, std::get<std::string>(m_detached).c_str());
    if (std::holds_alternative<bool>(m_detached))
        return yyjson_mut_bool(doc, std::get<bool>(m_detached));
    if (std::holds_alternative<ArrayData>(m_detached))
    {
        yyjson_mut_val* arr = yyjson_mut_arr(doc);
        for (auto& v : std::get<ArrayData>(m_detached))
            yyjson_mut_arr_append(arr, v.ToYYVal(doc));
        return arr;
    }
    if (std::holds_alternative<ObjectData>(m_detached))
    {
        yyjson_mut_val* obj = yyjson_mut_obj(doc);
        for (auto& kv : std::get<ObjectData>(m_detached))
            yyjson_mut_obj_add(obj, yyjson_mut_str(doc, kv.first.c_str()), kv.second.ToYYVal(doc));
        return obj;
    }
...
Value::UpdateTypeFromVal()
{
    ...
    if (yyjson_mut_is_null(m_val))
        m_type = kNullType;
    else if (yyjson_mut_is_bool(m_val))
        m_type = yyjson_mut_get_bool(m_val) ? kTrueType : kFalseType;
    else if (yyjson_mut_is_int(m_val) || yyjson_mut_is_uint(m_val))
        m_type = kNumberType;
...
template <typename Handler>
bool
Value::Accept(Handler& handler) const
{
    ...
    if (m_val)
    {
        ...
        if (yyjson_mut_is_bool(m_val))
            return handler.Bool(yyjson_mut_get_bool(m_val));
        if (yyjson_mut_is_int(m_val))
        {
            int64_t v = yyjson_mut_get_sint(m_val);
            if (v >= -2147483648LL && v <= 2147483647LL)
                return handler.Int((int)v);
        ...
        if (yyjson_mut_is_uint(m_val))
        {
            uint64_t v = yyjson_mut_get_uint(m_val);
            if (v <= 4294967295ULL)
                return handler.Uint((unsigned)v);
            return handler.Uint64(v);
        }
        if (yyjson_mut_is_real(m_val))
            return handler.Double(yyjson_mut_get_real(m_val));
        if (yyjson_mut_is_str(m_val))
            return handler.String(yyjson_mut_get_str(m_val), yyjson_mut_get_len(m_val), true);
        ...
        if (yyjson_mut_is_obj(m_val))
        {
            ...
            while ((key = yyjson_mut_obj_iter_next(&iter)))
            {
                ...
                if (m_children)
                {
                    ...
                    {
                        valToAccept = it->second.get();
                        // std::cout << "DEBUG: Accept FOUND OVERLAY for key=" << k << " val=" <<
                        // valToAccept << std::endl;
                    }
                }
                // else
                // std::cout << "DEBUG: Accept NO OVERLAY for key=" << k << std::endl;
    ...
    if (std::holds_alternative<int64_t>(m_detached))
        return handler.Int64(std::get<int64_t>(m_detached));
    if (std::holds_alternative<double>(m_detached))
        return handler.Double(std::get<double>(m_detached));
    if (std::holds_alternative<bool>(m_detached))
        return handler.Bool(std::get<bool>(m_detached));
    if (std::holds_alternative<std::string>(m_detached))
    {
    ...
    if (std::holds_alternative<ObjectData>(m_detached))
    {
        ...
        for (const auto& kv : obj)
        {
            if (!handler.Key(kv.first.c_str(), kv.first.size(), true))
                return false;
...
class Document : public Value
{
  public:
    ...
    {
        return m_type == kNullType && m_val == nullptr;
    }

    ParseErrorCode GetParseError() const
    {
        return HasParseError() ? kParseErrorDocumentEmpty : kParseErrorNone;
    }
...
GenericArrayIterator::operator[](difference_type n) const
{
    return (*m_parent)[(SizeType)(m_idx + n)];
}
