...
{
    NS_ASSERT_MSG(jModel.HasMember("name"), "Model configuration must have 'name' property.");
...
ModelConfigurationHelper::DecodeCoaleshedModel(const TypeId& model, const JsonObject& jModel)
{
    ...
    for (auto it = jModel.MemberBegin(); it != jModel.MemberEnd(); it++)
    {
        ...
        {
            NS_ASSERT_MSG(it->value.IsArray(), "'aggregates' must be an array of objects.");
            aggregates = DecodeModelAggregates(it->value.GetArray());
        }
        else
        {
            NS_FATAL_ERROR("Attribute '" << attrName << "' is not defined in model "
                                         << model.GetName());
        }
    }
                                               ...
                                               const TypeId::AttributeInformation& attrInfo)
{
    ...
    case rapidjson::Type::kArrayType: {
        ...
        else if (attrInfo.name == "FlightPlan" && arr[0].IsObject())
        {
            ...
            for (auto& p : arr)
            {
                NS_ASSERT_MSG(p.IsObject() && p.HasMember("position") && p["position"].IsArray() &&
                                  p["position"][0].IsDouble() && p.HasMember("interest") &&
                                  p["interest"].IsUint(),
                ...
                const double restTime = (p.HasMember("restTime") && p["restTime"].IsDouble())
                                            ...
                                            : 0;
                fp.Add(CreateObjectWithAttributes<ProtoPoint>("Position",
                                                              VectorValue(position),
                                                              "Interest",
                                                              IntegerValue(p["interest"].GetUint()),
                                                              "RestTime",
        ...
        {
            auto rect = Rectangle(arr[0].GetDouble(),
                                  arr[1].GetDouble(),
                                  arr[2].GetDouble(),
                                  arr[3].GetDouble());
        ...
        else if (attrInfo.name == "Patches" && arr[0].IsObject())
        {
            ...
            for (auto& p : arr)
            {
                const auto patchConfiguration =
                    DecodeCoaleshedModel(IrsPatch::GetTypeId(), p.GetObject());
                patches.push_back(patchConfiguration);
            }

            attrValue = attrInfo.checker->CreateValidValue(ModelConfigurationVectorValue(patches));
        }
        else if (attrInfo.name == "Configurations" && arr[0].IsArray() &&
                 arr[0].GetArray()[0].IsObject())
        {
        ...
        {
            NS_FATAL_ERROR("Unsupported attribute value type of array "
...
ModelConfigurationHelper::DecodeModelAttribute(const TypeId& model, const rapidjson::Value& el)
{
    ...
    NS_ASSERT_MSG(el.HasMember("name"), "Attribute model must have 'name' property.");
