  ...
  for (auto& netdev : json.GetArray ())
    {
      ...
      if (type == "wifi")
      ...
      else if (type == "lte")
        {
          ...
          const auto antennaModel = ModelConfigurationHelper::GetOptional (netdev.GetObject (), "antennaModel");
	        const auto phyTid = (role == "eNB")
                              ? LteEnbPhy::GetTypeId ()
                              : LteUePhy::GetTypeId ();
          const std::optional<ModelConfiguration> phyModel = ModelConfigurationHelper::GetOptionalCoaleshed (netdev.GetObject (), "phy", phyTid);

          confs.push_back (CreateObject<LteNetdeviceConfiguration> (type, role, bearers, networkLayerId, antennaModel, phyModel));
        }
      else
        {
