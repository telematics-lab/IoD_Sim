...
ReportPeripheral::ReportPeripheral(std::string tid,
                                   std::vector<double> powerConsumptionStates,
                                   std::vector<int> roi)
    : m_tid{tid},
...
ReportPeripheral::Write(xmlTextWriterPtr h)
{
    ...
    rc = xmlTextWriterWriteElement(h,
                                   BAD_CAST "OFF",
                                   BAD_CAST std::to_string(m_powerConsumptionStates[0]).c_str());
    NS_ASSERT(rc >= 0);
    rc = xmlTextWriterWriteElement(h,
                                   BAD_CAST "IDLE",
                                   BAD_CAST std::to_string(m_powerConsumptionStates[1]).c_str());
    NS_ASSERT(rc >= 0);
    rc = xmlTextWriterWriteElement(h,
                                   BAD_CAST "ON",
                                   BAD_CAST std::to_string(m_powerConsumptionStates[2]).c_str());
    NS_ASSERT(rc >= 0);
    ...
    {
        rc = xmlTextWriterWriteElement(h,
                                       BAD_CAST std::get<0>(attr).c_str(),
                                       BAD_CAST std::get<1>(attr).c_str());
        NS_ASSERT(rc >= 0);
    }
