...
namespace ns3
{
    ...
    void ReportEntity::Write(xmlTextWriterPtr h)
    {
        NS_LOG_FUNCTION(h);
    ...
    void ReportEntity::DoWrite(xmlTextWriterPtr handle)
    {
    }

    void ReportEntity::DoInitializeTrajectoryMonitor()
    {
    }

    void ReportEntity::DoMonitorTrajectory(const Ptr<const MobilityModel> mobility)
    {
    }

    void ReportEntity::DoInitialize()
    {
    ...
    void ReportEntity::DoInitializeDataStats()
    {
        NS_LOG_FUNCTION_NOARGS();
        ...
        {
            auto objRx =
                CreateObjectWithAttributes<ReportDataStats>("kind", PacketTypeValue(PacketType(i)));
            auto objTx =
                CreateObjectWithAttributes<ReportDataStats>("kind", PacketTypeValue(PacketType(i)));
    ...
    void ReportEntity::DoInitializeTrafficMonitors()
    {
        DoInitializeTrafficMonitor(TransferDirection("Rx"));
        DoInitializeTrafficMonitor(TransferDirection("Tx"));
    }

    void ReportEntity::DoInitializeTrafficMonitor(TransferDirection d)
    {
        NS_LOG_FUNCTION(d);
        ...
        auto connectSuccess = false;
        switch (d)
        {
        case TransferDirection::Value::Received:
            connectSuccess = Config::ConnectWithoutContextFailSafe(
                xPathCallback.str(),
                MakeCallback(&ReportEntity::DoMonitorRxTraffic, this));
            break;
        case TransferDirection::Value::Transmitted:
            connectSuccess = Config::ConnectWithoutContextFailSafe(
                xPathCallback.str(),
                MakeCallback(&ReportEntity::DoMonitorTxTraffic, this));
            break;
        }

        if (!connectSuccess)
            NS_LOG_WARN("Failed to connect callback to " << xPathCallback.str()
                                                         << ". Node IPv4L3Protocol is missing.");
    }

    void ReportEntity::DoMonitorRxTraffic(Ptr<const Packet> packet,
                                          Ptr<Ipv4> ipv4,
                                          uint32_t interface)
    {
        ...
        while (i.HasNext())
        {
            ...
            if (!item.isFragment && item.type == PacketMetadata::Item::PAYLOAD)
            {
                ...
                pCopy->CopyData(buf,
                                (item.currentSize > bufferLength) ? bufferLength
                                                                  : item.currentSize);
        ...
        if (payloadSize > 0)
        {
            ...
            auto reportTransfer = CreateObjectWithAttributes<ReportTransfer>(
                "EntityID",
                IntegerValue(m_reference),
                "Interface",
                IntegerValue(ipv4->GetInterfaceForAddress(ipv4dstaddr)),
                "PacketType",
                PacketTypeValue(packetType),
                "TransferDirection",
                TransferDirectionValue(direction),
                "Time",
                ...
                "SequenceNumber",
                IntegerValue(sequenceNumber),
                "Payload",
                StringValue(payloadDecoded));
            m_dataRx.push_back(reportTransfer);
        }
    }

    void ReportEntity::DoMonitorTxTraffic(Ptr<const Packet> packet,
                                          Ptr<Ipv4> ipv4,
                                          uint32_t interface)
    {
        NS_LOG_FUNCTION(packet << ipv4 << interface);
        ...
        if (payloadSize > 0)
        {
            ...
            auto reportTransfer = CreateObjectWithAttributes<ReportTransfer>(
                ...
                IntegerValue(m_reference),
                "Interface",
                IntegerValue(ipv4->GetInterfaceForAddress(ipv4srcaddr)),
                "PacketType",
                PacketTypeValue(packetType),
                "TransferDirection",
                TransferDirectionValue(direction),
    ...
    bool ReportEntity::IsWifiNetDevice(Ptr<NetDevice> device)
    {
    ...
    ReportEntity::GetIpv4Address(Ptr<const NetDevice> dev)
    {
        ...
        if (ifid == -1)
            return std::tuple<const int32_t, const std::string, const std::string>{-1, "", ""};
    ...
    void ReportEntity::DoInitializeNetworkStacks()
    {
        NS_LOG_FUNCTION_NOARGS();
        ...
        for (uint32_t i = 0; i < netdevN; i++)
        {
            ...
            if (ifid == -1)
                continue;
            while ((int)m_networkStacks.size() <= (int)ifid)
            ...
            if (IsWifiNetDevice(dev))
            {
                ...
                auto layer = CreateObjectWithAttributes<ProtocolLayer>(
                    "InstanceType",
                    StringValue(dev->GetInstanceTypeId().GetName()));
                auto phyLayer = CreateObjectWithAttributes<WifiPhyLayer>(
                    "Frequency",
                    IntegerValue(inspector.GetCarrierFrequency()),
                    "Standard",
                    StringValue(inspector.GetWifiStandard()),
                    "PropagationDelayModel",
                    StringValue(inspector.GetPropagationDelayModel()),
                    "PropagationLossModel",
                    StringValue(inspector.GetPropagationLossModel()),
                    "NodeId",
                    IntegerValue(m_reference),
                    "NetdevId",
                    IntegerValue(i));
                auto macLayer =
                    CreateObjectWithAttributes<WifiMacLayer>("Ssid",
                                                             StringValue(inspector.GetWifiSsid()),
                                                             "Mode",
                                                             StringValue(inspector.GetWifiMode()));
                auto ipv4Layer = CreateObjectWithAttributes<Ipv4Layer>(
                    "Ipv4Address",
                    StringValue(std::get<1>(ipv4AddressMask)),
                    "SubnetMask",
                    StringValue(std::get<2>(ipv4AddressMask)));
                auto droneControlLayer =
                    CreateObjectWithAttributes<DroneControlLayer>("NotImplemented", StringValue());
            ...
            else if (dev->GetInstanceTypeId().GetName() == "ns3::LteUeNetDevice")
            {
                ...
                auto pldltype = ccmap->GetPhy()
                                    ->GetDlSpectrumPhy()
                                    ->GetChannel()
                                    ->GetPropagationLossModel()
                                    ->GetInstanceTypeId()
                                    .GetName();
                auto plultype = ccmap->GetPhy()
                                    ->GetUlSpectrumPhy()
                                    ->GetChannel()
                                    ->GetPropagationLossModel()
                                    ->GetInstanceTypeId()
                ...
                auto phyLayer = CreateObjectWithAttributes<LteUEPhyLayer>(
                    "IMSI",
                    StringValue(std::to_string(lteuenetdev->GetImsi())),
                    "PropagationLossModelDl",
                    StringValue(pldltype),
                    "PropagationLossModelUl",
                    StringValue(plultype),
                    "DlEarfcn",
                    StringValue(std::to_string(ccmap->GetDlEarfcn())),
                    "DlBandwidth",
                    StringValue(std::to_string(ccmap->GetDlBandwidth())),
                    "UlEarfcn",
                    StringValue(std::to_string(ccmap->GetUlEarfcn())),
                    "UlBandwidth",
                    StringValue(std::to_string(ccmap->GetUlBandwidth())));
