...
OUTPUT_FILE_PREFIX=periph-power
DEBUG_LOG_FILE=$(basename $1)
RESULTS_DIR=$(dirname $1)
N_DRONES=$(grep -oP '(?<=Peripherals Power Consumption on Drone #)([0-9]+)' $RESULTS_DIR/$DEBUG_LOG_FILE \
            | sort -nr \
            | head -n1)
...
for i in $(seq 0 $N_DRONES); do
  ...
  grep "Peripherals Power Consumption on Drone #${i}" $RESULTS_DIR/$DEBUG_LOG_FILE \
