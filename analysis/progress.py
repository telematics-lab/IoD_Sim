...
rex = re.compile(
    r".*\( +(?P<speedup>[0-9\.]+)x real time\) (?P<events>[0-9]+) events processed"
)

samples = [
    [m.groupdict() for m in filter(None, map(rex.match, f.open(mode="r").readlines()))]
    for f in filenames
]
...
fig = go.Figure(data=[go.Box(y=p["speedup"]) for p in performances])
fig.show()

# %%
#!BUG: asse Y non ordinato!
fig = go.Figure(data=[go.Box(y=p["events"]) for p in performances])
fig.show()
