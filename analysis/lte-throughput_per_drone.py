...
P = ArgumentParser(description='Parse LTE MAC Ul stats per drone to evaluate its throughput')
  ...
  with open(f'{args.results_dir}/{INPUT_CSV_PREFIX}_{i_drone}.csv') as csvf:
    ...
    for r in csvr:
      ...
      if int(stime) > timeref:
        ...
        drones[i_drone].append([int(timeref), scellid, txbytes * 8.0 / (1024.0 ** exp)])
        timeref = stime
        txbytes = 0.0
