...
wifi_tx_trace_filenames = [f"wifi-phy-0-drones-host-{i}-0.log" for i in range(4, 10)]
wifi_tx_trace_filenames += [f"wifi-phy-1-drones-host-{i}-0.log" for i in range(10, 20)]
wifi_tx_trace_filenames += [f"wifi-phy-2-drones-host-{i}-0.log" for i in range(20, 30)]
wifi_tx_trace_filenames += [f"wifi-phy-3-drones-host-{i}-0.log" for i in range(30, 38)]
...
##%
rex = re.compile(
    r"t (?P<time>[0-9\.+]+).+HeMcs(?P<mcs>[0-9]+).+ (?P<ipaddr>[0-9\.]+) > 200\.0\.0\.1.+ns3::SeqTsHeader \(\(seq=(?P<sn>[0-9]+).+"
)
...
with open(f"{base_results_dir}/{scenario_dir}/{internet_rx_trace_filename}", "r") as f:
    for l in f:
        ...
        if r is not None:
            ...
            pkt = {"time": float(g["time"]), "sn": float(g["sn"])}
...
for i in range(len(tx_traces)):
    ...
    latency[tx_traces[i]["ipaddr"][0]] = (
        lat["time_y"] - lat["time_x"]
    ) * 1e3  # to milliseconds

# %% Export data
lat2matlab = {"ClusterID": [], "IPAddr": [], "latency": []}
...
# %%
color_palette = ["rgb(0,0,0)" for i in range(2, 9)]
color_palette += ["rgb(0.5,0.5,0.5)" for i in range(2, 8)]
color_palette += ["rgb(0.7,0.7,0.7)" for i in range(2, 9)]
color_palette += ["rgb(0.7,0.7,0.7)" for i in range(2, 6)]
