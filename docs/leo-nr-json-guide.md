...
### `internetBackbone`
**Type:** `object`
**Description:** Configuration of the simulated internet backbone that connects remotes and network gateways.

| Parameter | Type | Description | Default |
|-----------|------|-------------|---------|
| `dataRate` | string | The DataRate of the CSMA link used for the backbone. | `"100Gbps"` |
| `delay` | string | The delay of the CSMA link used for the backbone. | `"0ms"` |
| `ipv4Base` | string | The base IPv4 address for the backbone network. | `"200.0.0.0"` |
| `ipv4Mask` | string | The network mask for the backbone IPv4 network. | `"255.0.0.0"` |
...
The `IoD-Sim` repository includes several python scripts and tools to help you design, analyze, and benchmark your LEO-NR scenarios.

### 1. Visualization & Analysis
- **`analysis/leo-geo-view.py`**: A very useful tool for visualizing the geographic layout, orbits, and movements of your nodes (satellites and ground vehicles) using the scenario JSON configuration.
- **`analysis/plot-leo-nr-ue-stats.py`**: A plotting tool dedicated to parsing and visualizing UE (User Equipment) statistics from your NR-LEO simulations, making it easier to analyze throughput, latency, and packet loss.

### 2. Scenario Generation
- **`tools/generate-leo-congestion-scenario.py`**: Automatically generates LEO congestion example configurations. This script is handy for creating heavy-load scenarios to test the limits of your topology or the NR MAC schedulers.

### 3. Benchmarks
Inside the `tools/leo-benchmarks/` directory, you will find scripts used to benchmark the performance and execution time of the simulator under varying conditions.
> Before running the benchmarks, you must compile the simulator using the provided Docker container (in compile folder) to ensure a clean, reproducible, and optimized build environment.
- **`tools/leo-benchmarks/run_bench.py`**: Executes the benchmark suite.
- **`tools/leo-benchmarks/plot_benchmarks.py`**: Plots the results generated by the benchmark runs, providing visual comparisons of execution times or other metrics.