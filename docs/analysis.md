...
- **isl-path-snapshot.py**: Static PNG snapshots of the route one UE's traffic takes to the core through the constellation. For each requested instant it draws the globe, the satellites, the whole ISL routing tree, the UE and its radio link, and highlights the hop-by-hop path from the serving satellite down to the ground station that fronts the core, captioned with the hop distances and the backhaul delay applied. Takes a results directory; `--times` picks the instants (by default the ones at which the route changes shape), `--ue` picks the terminal. Writes into the run's `graphs/` subfolder. The interactive, animated equivalent is `leo-geo-view.py`.
...
- **plot-isl-delay-experience.py**: Relates the delay a user actually experiences to the delay the ISL model introduces. For one run it draws the UE's uplink and downlink end-to-end application delay next to the backhaul delay applied to its serving satellite, on a common millisecond axis, with the hop count below and handovers marked. Given several runs that differ only in `additionalDelay` it also plots experienced delay against that introduced delay, with a slope-1 reference. Takes one or more results directories and writes into the first one's `graphs/` subfolder.
